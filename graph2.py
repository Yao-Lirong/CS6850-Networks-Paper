...
Graph with Normal edges and Inverse edges
...
class Graph:
	...
	def add_edge(self, frm, to, normal = None):
		...
		"""
		if normal == None: normal = rand_bool()
		u = self.nodes[frm]
		v = self.nodes[to]
	...
	def update_graph_add(self):
		...
		v_black_num = self.get_black_num(self.nodes[v])
		# Note the Pr[v receives a black input] \prop #[current black inputs]
		# If u is Black, Pr[edge is normal] \equiv Pr[v receives a black input]
		edge_color_weights = [v_black_num, len(self.nodes[v].in_edges) - v_black_num]
		# If u is White, Pr[edge is inverse] \equiv Pr[v receives a black input]
		if node_u.current_color == False: edge_color_weights.reverse()
		edge_color = random.choices([True, False], edge_color_weights)[0]
		self.add_edge(u, v, edge_color)

		if DEBUG:
			str = "normal" if edge_color else "inverse"
			print("Added a " + str + " edge (%d, %d)" % (u, v))
		return True
